...
async function startServer() {
    ...
    app.post("/api/github/push-commit", async (req, res) => {
        try {
            const authHeader = req.headers.authorization;
            const { token: bodyToken, targetOwner, repo = "contratos-inteligentes", branch = "main", filePath = "programs/solana_sandbox_counter/src/lib.rs", fileContent, commitMessage = "DevSecOps: update Rust smart contract with AST security fixes", } = req.body;
            const token = authHeader?.replace("Bearer ", "") || bodyToken;
            ...
            const getFileRes = await fetch(`https://api.github.com/repos/${targetOwner}/${repo}/contents/${filePath}?ref=${branch}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    Accept: "application/vnd.github.v3+json",
                    "User-Agent": "Solana-DevSecOps-IDE",
